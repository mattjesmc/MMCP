# The modder's session charter

What an agent session in a MOD repository should know before its first tool call, written to be
pasted into that repository's `CLAUDE.md` (or whichever project-context file the agent client
reads). It is the developer counterpart of the copilot sheet the in-game launcher writes
(`CLAUDE.md.tpl`) and the survival player's `SURVIVAL_CHARTER.md`: those two describe sessions
that live in the GAME directory; this one describes a session that lives in YOUR repository and
reaches the game through the toolkit's MCP server.

The first paragraph is also what the MCP server itself serves as its `instructions` when the
project's `.mcptoolkit/loop.json` does not supply one, so a session with no charter at all still
meets it.
